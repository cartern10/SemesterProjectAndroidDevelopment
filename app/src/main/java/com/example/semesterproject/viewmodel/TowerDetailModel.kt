...
@HiltViewModel
class TowerDetailModel @Inject constructor(
    ...
    fun fillData(name: String) {
        viewModelScope.launch {
            when (val response = towerRepository.getSpecificTower(name)) {
                TowerResponse.Error -> _towers.value = TowerDetailModel.TowerEvent.Failure
